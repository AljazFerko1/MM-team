...
%! res = [[1,0,0]; [0,1,0]; [0,0,1]; [0.5774,0.5774,0.5774]; [-0.5774,-0.5774,-0.5774]; [0,0,0]];
%! assert(move_inside(Xs, func_s, grad_s, @out_sphere, max_iter=10), res, 1e-4)
%!test
%! func_e = @(x,y,z) [(x.^2)/2 + (y.^2)/2 + (z.^2)/4 - 1];
%! grad_e = @(x,y,z) [x, y, z/2];
%! Xs = [[3,0,0]; [0,3,0]; [0,0,3]; [1,1,1]; [-2,-2,-2]; [0,0,0]];
%! res = [[1.4142,0,0]; [0,1.4142,0]; [0,0,2]; [0.8824, 0.8824,0.9412]; [-0.7307,-0.7307,-1.3654]; [0,0,0]];
%! assert(move_inside(Xs, func_e, grad_e, @out_ellipsoid, max_iter=10), res, 1e-4)