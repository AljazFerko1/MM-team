...
%! res = [[-0.2192,-0.2192,-0.2192];[1.1366,1.1366,1.1366];[3.0481,3.0481,3.0481];[6.0344,6.0344,6.0344]];
%! assert(forces(Xs, 1), res, 1e-4)
%!test
%! Xs = [[1,2,3];[4,4,4];[5,6,7]];
%! res = [[0.9307,1.9498,2.9689];[4.0382,4,3.9618];[5.0311,6.0502,7.0693]];
%! assert(forces(Xs, 1), res, 1e-4)
%!test
%! Xs = [[1,2,3];[4,4,4];[5,6,7]];
%! res = [[0.9931,1.9950,2.9969];[4.0038,4,3.9962];[5.0031,6.0050,7.0069]];
%! assert(forces(Xs), res, 1e-4)
%!test
%! Xs = [[0,0,1];[0,0,-1]];
%! res = [[0,0,1.0025];[0,0,-1.0025]];
%! assert(forces(Xs, 0.01), res, 1e-4)